...
SERIAL_LOG="${SERIAL_LOG:-LOGs/xv6-largewrite-serial.log}"
mkdir -p LOGs
QEMU_OUT="${QEMU_OUT:-/tmp/xv6-largewrite-qemu.out}"
...
/bin/xv6_largewrite_test.elf
EOF

make "${ISO}" >/tmp/xv6-largewrite-build.out
...
echo "Waiting for xv6 large write smoke..."
for _ in $(seq 1 120); do
    if grep -q 'xv6-largewrite-smoke: PASS' "${SERIAL_LOG}" 2>/dev/null; then
        break
    fi
    sleep 1
done
...
if ! grep -q 'xv6-largewrite-smoke: PASS' "${SERIAL_LOG}"; then
    echo "xv6 large write smoke FAIL: pass marker not found"
...
echo "xv6 large write smoke PASS"
tail -n 160 "${SERIAL_LOG}"
