...
SERIAL_LOG="${SERIAL_LOG:-LOGs/pread-pwrite-serial.log}"
mkdir -p LOGs
QEMU_OUT="${QEMU_OUT:-/tmp/pread-pwrite-qemu.out}"
...
/bin/preadpwrite_test.elf
EOF

make "${ISO}" >/tmp/pread-pwrite-build.out
...
echo "Waiting for pread/pwrite smoke..."
for _ in $(seq 1 120); do
    if grep -q 'preadpwrite-smoke: PASS' "${SERIAL_LOG}" 2>/dev/null; then
        break
    fi
    sleep 1
done
...
if ! grep -q 'preadpwrite-smoke: PASS' "${SERIAL_LOG}"; then
    echo "pread/pwrite smoke FAIL: pass marker not found"
