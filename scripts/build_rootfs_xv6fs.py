...
MTIME_MAX    = 0xFFFFFFFF              # inode の mtime は 32bit 無符号
...
def _load_image(img_path: str) -> int:
    ...
    **サイズと inode 数も superblock から取る。**これらを道具側の定数
    (既定 or XV6FS_FSSIZE/XV6FS_NINODES) のままにすると、イメージを作った
    ときと読むときで食い違う。rootfs も smoke も作るときだけ環境変数を
    渡し (例: XV6FS_NINODES=256)、--replace/--extract/--check は渡さずに
    呼ぶので、**この食い違いは常に起きていた**。実害の例:
      - _max_mtime_in_image() が inode 領域を越えてデータ領域をゴミ inode
        として読み、mtime に 0xffffffff を拾って --replace が
        struct.error で落ちる (2026-09-04 に再現)
      - check_image() の nmeta / FSSIZE によるブロック範囲検査が的外れになる
    """
    global image, inodestart, bmapstart, logstart
    global FSSIZE, NINODES, nblocks, nlog, ninodeblocks, nbitmap, nmeta

    raw = Path(img_path).read_bytes()
    image = bytearray(raw)

    sb_fields = struct.unpack("<IIIIIIII", bytes(image[BSIZE:BSIZE + 32]))
    (magic, size_sb, nblocks_sb, ninodes_sb, nlog_sb,
     logstart_sb, inodestart_sb, bmapstart_sb) = sb_fields
    ver = format_from_magic(magic)
    if ver == 0:
        print(f"ERROR: bad magic 0x{magic:08x} (expected 0x{FSMAGIC:08x})", file=sys.stderr)
        return 1
    ...
    # 派生する数も superblock から取り直す。ninodeblocks は
    # bmapstart = 2 + nlog + ninodeblocks という置き方の逆算で出る
    FSSIZE       = size_sb
    NINODES      = ninodes_sb
    nblocks      = nblocks_sb
    nlog         = nlog_sb
    ninodeblocks = bmapstart - inodestart
    nbitmap      = FSSIZE // (BSIZE * 8) + 1
...
def _max_mtime_in_image() -> int:
    ...
    **返り値は 32bit に収まる範囲へ落とす** (保険)。呼び出し側は +1 して
    inode へ書き戻すので、0xffffffff をそのまま返すと struct.pack の
    'I' が範囲外で落ちる。_load_image が superblock を反映するようになって
    inode 領域を踏み越えることは無くなったが、壊れたイメージを読んだとき
    に無関係な例外で止まらないようにここでも留める"""
    hi = 0
