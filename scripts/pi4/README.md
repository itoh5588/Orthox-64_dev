...
**このケーブルは信号が論理反転している。**素のままでは化けた文字しか出ない。
FT_PROG での対処は下の「変換器が信号を反転していることがある」を読むこと。
...
設計だった。**その EEPROM 設定なしで出荷されていた。**
    ...
    Pi (正論理)  →  ケーブルの反転回路  →  FT232R の Invert RXD  →  PC
         正              反転                    もう一度反転           正

### ループバックでは絶対に見つからない

TX と RX を直結して打ち返す試験は、**TX の反転と RX の反転が打ち消し合う**ので
正常に見える。**この方法で切り分けようとしないこと。**
...
4. **`Invert TXD` と `Invert RXD` にチェックを入れる** — **外すのではない**
...
| EMMC2 | `0xFE340000` irq 158 (dtb) | 番地は取れたが**コマンドが通らない** (下記) |
| timer freq | 54 MHz | 想定どおり |
| cpus | 4 | |

**boot / dtb / timer / mmu / shared / user / sched はすべて ok。**
MMU も EL0 も実機で無修正のまま動いた。

## 実機で残っている問題

- **`emmc2 : CMD0 に応答しない`。**番地と irq は DTB から正しく取れているが、
  最初のコマンドで返らない = **そもそもコントローラが起きていない**可能性が高い。
  `kernel/aarch64/emmc2.c` の冒頭にあるとおり、VideoCore のメールボックスでの
  **電源／クロック有効化が抜けている**線が濃厚 (日報2026-08-15 §4)
...
- **EMMC2。** QEMU の raspi4b は SD を旧 sdhci (0xFE300000) に繋いでいて
  EMMC2 は空 (日報2026-08-13 §6)。**実機では番地は取れたが CMD0 が返らない**
