 ...
 *
 * **kernel/net_socket.c は TCP/UDP をフル実装しているのに、Linux ABI の
 * 入口が無かった。** x86 だけが独自 syscall 経路 (kernel/sys_net.c) から
 * 呼んでおり、aarch64/riscv64 のユーザー空間からはソケットが使えなかった。
 * 番号は asm-generic (aarch64/riscv64 共通)。値は
 * scripts/check_riscv64_syscalls.py が musl の __NR_* と突き合わせる */
...
/* sendmsg/recvmsg (2026-09-05)。**musl の resolver が recvmsg を使う** —
 * これが無いと getaddrinfo が ENOSYS で止まり、名前で繋げない */
