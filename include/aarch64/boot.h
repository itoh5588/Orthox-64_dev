...
typedef struct aarch64_boot_info {
    ...
    /* Raspberry Pi 4 (BCM2711) の実 NIC (GENETv5)。**QEMU virt には無い**
     * ので 0 のまま (virtio-net-mmio を使う)。N-9 手2 で追加。
     * phy_mode は include/aarch64/boot.h ではなく genet.c 側の enum に
     * 合わせた小さい整数 (DTB の phy-mode 文字列から変換したもの)。
     * mac が全部 0 なら DTB に local-mac-address が無かった —
     * その場合はレジスタに既に入っている値 (ファームウェアが焼いたもの)
     * を使う */
    uint64_t genet_base;
    uint64_t genet_size;
    uint32_t genet_intid;
    uint32_t genet_phy_mode;
    uint8_t  genet_mac[6];
    /* /scb の dma-ranges から。**複数組のうち一番 size が大きいものを選ぶ**
     * (kernel/aarch64/dtb.c 参照)。実機 (2026-09) は offset=0 (恒等)、
     * limit=16GB — RX/TX バッファの物理アドレスをそのまま渡してよい */
    uint64_t genet_dma_offset;
    uint64_t genet_dma_limit;
    /* BCM2711 の乱数器 (RNG200)。**QEMU virt には無い**ので 0 のまま
     * (そちらは virtio-rng を探す)。TLS が鍵の材料をここから取る。
     * 2026-09-05 に追加 */
    uint64_t rng_base;
    uint64_t rng_size;
    uint32_t flags;
...
#define AARCH64_BOOT_FLAG_GENET_FROM_DTB  (1U << 14)
#define AARCH64_BOOT_FLAG_RNG_FROM_DTB    (1U << 15)
#define AARCH64_BOOT_FLAG_GENET_MAC_FROM_DTB (1U << 15)
