...
#include <stdlib.h>
#include <signal.h>
/* **ORTHOX_LINUX_ABI は aarch64/riscv64 のビルドで立てる。**
 *
 * include/syscall.h は x86 の独自 syscall 経路のヘッダで、musl と一緒には
 * 使えない (sigaction の sa_handler が musl のマクロと衝突してコンパイル
 * できない)。x86 はこのヘッダの dns_lookup_ipv4 を使って名前を引くが、
 * **Linux ABI にはそれに当たる番号が無い** (Linux は名前解決を libc が
 * UDP でやる)。当面は IP 直指定だけ通し、名前は 手3 で /etc/resolv.conf +
 * musl の resolver に繋ぐ (2026-09-05) */
#ifdef ORTHOX_LINUX_ABI
#include <stdint.h>
#include <netdb.h>
/* **Linux と同じ形で引く。**名前解決は libc の仕事で、/etc/resolv.conf に
 * 書いてあるサーバへ UDP で問い合わせる。UDP のソケットは N-10 で EL0 から
 * 使えるようになっている。resolv.conf は DHCP が終わったところで
 * カーネルが書く (kernel/lwip_port.c) */
static int dns_lookup_ipv4(const char *hostname, uint32_t *out_addr) {
    ...
    hints.ai_family = AF_INET;          /* IPv6 は持っていない */
    hints.ai_socktype = SOCK_STREAM;
    rc = getaddrinfo(hostname, 0, &hints, &res);
...
/* **接続先 (addr) と証明書を照合する名前 (host) を分けている。**
 *
 * connect_addr が 0 なら従来どおり host を引いて繋ぐ。渡すとそこへ繋ぎ、
 * SNI と証明書の照合には host を使う。名前解決がまだ無い環境 (aarch64、
 * 手3 の前) で本物の証明書検証を通したまま試すために要る。**検証を
 * 緩めるものではない** —— 名前が合わなければ今までどおり弾かれる */
static int fetch_once(const char *host, int port, const char *path,
    const char *connect_addr, struct http_buffer *raw, int *out_ssl_err, int *out_x509_err)
{
    int fd;
    unsigned long ip = 0;
    ...
    {
        const char *target = connect_addr ? connect_addr : host;
        if (parse_ipv4(target, &ip) < 0) {
            if (dns_lookup_ipv4(target, (uint32_t *)&ip) < 0) {
...
int main(int argc, char **argv)
{
    ...
    /* 第 4 引数は繋ぎ先の IP。省くと host をそのまま引く */
    addr = (argc >= 5) ? argv[4] : 0;
    if (argc >= 3) {
        port = atoi(argv[2]);
    ...
    for (;;) {
        if (fetch_once(host, port, path, addr, &raw, &ssl_err, &x509_err) < 0) {
            free(raw.data);
            free(decoded.data);
            free(host);
            free(path);
