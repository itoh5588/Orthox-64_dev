...
#include "arch_time.h"
#include "task.h"
#include "arch_bootmod.h"
#include "linux_syscall.h"   /* arch_random_fill: /dev/urandom の中身 */
#include "pmm.h"
#include "vmm.h"
#include "usb.h"
...
static int build_module_dirents(const char* dir_path, struct orth_dirent* dirents,
    ...
    const uint64_t nmod = arch_boot_module_count();
    if (nmod == 0) return -1;

    for (uint64_t i = 0; i < nmod; i++) {
        struct boot_module m = arch_boot_module_at(i);
        char child[248];
...
static int build_root_dirents(struct orth_dirent* dirents, size_t max_entries, size_t* out_count) {
    ...
    } else if (arch_boot_module_count() > 0) {
    ...
    if (fs_fd_type(f) == FT_CHARDEV) {
        ...
        if (major == CHR_MAJOR_MEM) {
            ...
            if (minor == CHR_MINOR_RANDOM || minor == CHR_MINOR_URANDOM) {
                /* **getrandom(2) ではなく arch_random_fill (2026-09-09)。**
                 * ここは **エラーを返さない口** —— getrandom(2) が ENOSYS の
                 * ときに呼び手が退避してくる先なので、同じ源を見ていると
                 * 退避先ごと塞がる。**RDRAND の無い x86 と riscv64 で
                 * python3 が起動しなかったのはこれ**
                 * (include/linux_syscall.h の表を参照)。
                 * Linux も 5.6 以降は /dev/random をブロックさせないので、
                 * 2 つを分けていない */
                arch_random_fill(buf, count);
                return (int64_t)count;
            }
        }
        return -EIO;
...
void fs_free_exec_buffer(const char* path, void* data, size_t size) {
    ...
    /* ブートモジュールの中身はブートローダが確保した領域そのもの。
     * **呼び手の所有ではない**ので解放しない (include/boot_module.h)。 */
    const uint64_t nmod = arch_boot_module_count();
    if (nmod > 0) {
        for (uint64_t i = 0; i < nmod; i++) {
            struct boot_module m = arch_boot_module_at(i);
