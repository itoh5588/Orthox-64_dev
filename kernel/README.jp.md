...
## 置き場が層を表す (2026-09-07 に整理)

    kernel/           **ISA に依らない層** + まだ統合していない syscall 層
    kernel/x86_64/    x86_64 の実装。**ここに置いてあるものが x86 依存**
    kernel/riscv64/   riscv64 の実装
    kernel/aarch64/   aarch64 の実装 (Raspberry Pi 4 実機 / QEMU virt)

**それまでは kernel/ 直下が「共通の層」と「x86 の実装」を兼ねていた。**
どちらの層なのかは Makefile の `SRCS` / `AARCH64_SHARED_C_SRCS` /
`RISCV64_SHARED_C_SRCS` を読むまで分からず、**同名の別実装が 18 種**あった。
下の一覧では、x86 の実装に移したものに `x86_64/` を付けて示す。

**`syscall.c` と `sys_*.c` は kernel/ 直下に残してある。**x86 だけが使って
いるが x86 依存ではなく、同じ syscall を aarch64 / riscv64 は
`linux_syscall.c` で実装している (二重実装。統合は未了)。
