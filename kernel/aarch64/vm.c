...
static void aarch64_vm_build_kernel(void) {
    ...
    /* Pi 4 の実 NIC (GENETv5、N-9 手2)。**emmc2 と同じ形で張り忘れると
     * translation fault になる**(実測: FAR=genet_base+0x808 の
     * GENET_UMAC_CMD 読み出しで level 3 fault、2026-09-04) */
    if (b->genet_base) {
        uint64_t sz = b->genet_size ? b->genet_size : AARCH64_PAGE_SIZE;
        if (sz < AARCH64_PAGE_SIZE) sz = AARCH64_PAGE_SIZE;
        aarch64_vm_map_range(aarch64_phys_to_virt(b->genet_base), b->genet_base,
                             sz, VM_DEVICE_RW);
    }

    /* Pi 4 の乱数器 (RNG200、2026-09-05)。genet と同じ形。**張り忘れると
     * 最初の 1 語を読んだところで translation fault になる** */
    if (b->rng_base) {
        uint64_t sz = b->rng_size ? b->rng_size : AARCH64_PAGE_SIZE;
        if (sz < AARCH64_PAGE_SIZE) sz = AARCH64_PAGE_SIZE;
        aarch64_vm_map_range(aarch64_phys_to_virt(b->rng_base), b->rng_base,
                             sz, VM_DEVICE_RW);
    }
