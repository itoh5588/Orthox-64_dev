...
/* **時刻の見せ方 (2026-09-05)。**
 *
 * カーネルは CLOCK_REALTIME を **UTC のまま持つ** (SNTP がそう返す)。
 * **時計そのものを +9 時間ずらしてはいけない** —— TLS が証明書の有効期間を
 * その時計で判定するので、ずらすと期限の判定が 9 時間ぶん狂う。
 * 変えるのは見せ方だけ。
 *
 * musl は tzdata (/usr/share/zoneinfo) が無くても **POSIX 形式の TZ なら
 * 自分で解釈する**。`Asia/Tokyo` は tzdata が要るので実機では効かない
 * (2026-09-05 に実機で確認: UTC のままだった)。
 *
 * `JST-9` の符号は POSIX の決まりで、**UTC から見て西向きが正**。
 * 東経 9 時間の日本は `-9` と書く。
 *
 * 別の地域にするとき: make aarch64-pi4-netboot ORTHOX_TZ=CET-1 */
#ifndef ORTHOX_TZ
#define ORTHOX_TZ "JST-9"
#endif

/* **PATH。**実機の rootfs は busybox 群が /bin、ツールチェーン
 * (gcc / as / ld / make) が /usr/bin にある。**入っていないと
 * フルパスで打つことになる。**
 *
 * **存在しないディレクトリが並んでいても害はない** (smoke 用の小さい
 * イメージには /usr/bin が無いが、探して見つからないだけ)。
 *
 * 変えるとき: make aarch64-pi4-netboot ORTHOX_PATH=/bin:/usr/bin:/opt/bin */
#ifndef ORTHOX_PATH
#define ORTHOX_PATH "/bin:/usr/bin"
#endif

int aarch64_first_user_task(void) {
    static char* argv[] = { (char*)AARCH64_INIT_PATH, 0 };
    /* **子プロセスにも継がれる。**ash から起動したものは全部この帯で出る */
    static char* envp[] = { (char*)"TZ=" ORTHOX_TZ,
                            (char*)"PATH=" ORTHOX_PATH, 0 };
 ...
 * USB は aarch64 側でポーリング運用のため、bottom half の仕組みは未使用 */
int bottom_half_run(void) {
    return 0;
}

/* net_poll()/net_needs_poll_fallback()/net_init() の実体は
 * kernel/net.c (共有層、N-9 手1 で aarch64 にも繋いだ)。
 * ここには置かない — 置くと多重定義になる */
