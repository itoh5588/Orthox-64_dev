...
/* ---- 呼び手のバッファに直接 DMA しない (2026-09-06) ---------------------
 *
 * **desc[1].addr は aarch64_virt_to_phys(buf) で作っている。**これは
 * カーネル線形マップ専用の引き算 (va - 0xffffff8000000000) なので、
 * 線形マップの外を指すバッファでは成り立たない。
 *
 * 実際に踏んだ形 (2026-09-06):
 *
 *   read(2) はユーザのバッファをそのまま xv6fs_readi へ渡す
 *   (kernel/fs.c)。2 ブロック以上まとめて読む区間は P-5 の最適化で
 *   キャッシュを迂回して xv6bio_rw_run -> ここへ来る。**ユーザ VA は
 *   0x400000 のような低い番地**なので、引き算が桁を回って出鱈目な
 *   物理アドレスになり、QEMU が
 *
 *     virtio: bogus descriptor or out of resources
 *
 *   を出して要求を捨てる。完了割り込みが来ないので -2 (時間切れ) で
 *   返り、そこから xv6fs 全体が壊れていった。
 *   **症状は「8KB 以上のファイルをユーザ空間で読むと必ず死ぬ」。**
 *
 * **EMMC2 は最初からこれを避けている** (kernel/aarch64/emmc2.c の
 * 「呼び手のバッファに直接 DMA しない」)。同じ形に揃える。
 *
 * **例外を作らず、常に枠を経由する。** 最初は「線形マップの中なら
 * そのまま渡す」形にしたが、それでも 2026-09-06 に
 * `block=1460 n=9` で同じ bogus を踏んだ —— **上位 VA だからといって
 * 線形マップとは限らない。** 判定を持つと、当たっているかどうかが
 * 症状からは分からない。EMMC2 と同じく無条件で写す。
 *
 * 代償は memcpy 1 回。要求そのものの往復 (notify + 割り込み待ち) の
 * ほうがはるかに重いので、実測できる差にはならない。 */
#define VBLK_BOUNCE_SECTORS 16U   /* 8KB。1 回の往復で運ぶ上限 */
static uint8_t g_bounce[VBLK_BOUNCE_SECTORS * VBLK_SECTOR_SIZE]
    __attribute__((aligned(16)));

static void vblk_memcpy(void* dst, const void* src, uint64_t n) {
    uint8_t* d = (uint8_t*)dst;
    const uint8_t* s = (const uint8_t*)src;
    for (uint64_t i = 0; i < n; i++) d[i] = s[i];
}

static int vblk_rw_bounced(uint32_t type, uint64_t sector, void* buf,
                           uint32_t sectors) {
    uint8_t* p = (uint8_t*)buf;
    while (sectors > 0) {
        uint32_t n = sectors < VBLK_BOUNCE_SECTORS ? sectors : VBLK_BOUNCE_SECTORS;
        uint64_t bytes = (uint64_t)n * VBLK_SECTOR_SIZE;
        if (type == VIRTIO_BLK_T_OUT) vblk_memcpy(g_bounce, p, bytes);
        int ret = vblk_rw_locked(type, sector, g_bounce, n);
        if (ret != 0) return ret;
        if (type == VIRTIO_BLK_T_IN) vblk_memcpy(p, g_bounce, bytes);
        p += bytes;
        sector += n;
        sectors -= n;
    }
...
static int vblk_rw(uint32_t type, uint64_t sector, void* buf, uint32_t sectors) {
    ...
    ret = vblk_rw_bounced(type, sector, buf, sectors);
