...
/* virtio-net のポーリングと lwIP のタイムアウト処理 (kernel/net.c、N-9 手1)。
 * ネットワークが見つかっていない機械では中で早期に戻る */
void net_poll(void);
...
static void cpu_stats_report(void) {
    ...
    /* **UART 受信エラーフラグ。**増えた区間だけ 1 行出る (日報2026-09-16 §4) */
    aarch64_uart_rx_err_report();
...
void aarch64_timer_on_tick(struct aarch64_trap_frame* frame) {
    ...
    if (me == 0) {
        ...
        /* virtio-net のポーリングと lwIP のタイムアウト処理 (N-9 手1)。
         * kbd_tick/sound_tick と同じ理由で CPU 0 だけ。ネットワークが
         * 見つかっていない機械 (実機) では net_needs_poll_fallback() が
         * 0 を返すのでほぼ即座に戻る */
        net_poll();
