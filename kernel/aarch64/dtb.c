...
typedef struct dtb_node_state {
    ...
    int is_pcie_brcm;
    /* Raspberry Pi 4 の実 NIC (GENETv5)。N-9 手2 */
    int is_genet;
    int is_rng;
    int is_memory;
    int is_timer;
    ...
    uint32_t intr_len;
    /* GENET の PHY 接続方式 ("rgmii"/"rgmii-id"/"rgmii-rxid"/"rgmii-txid")。
     * 直書きしない — Pi 4 の実配線に合わせて DTB が決めている値を読む */
    const uint8_t* phy_mode_data;
    uint32_t phy_mode_len;
    /* GENET の MAC アドレス。ファームウェアが OTP から埋める。無ければ
     * このノードには無いということなので、後でレジスタの値を使う */
    const uint8_t* mac_addr_data;
...
void aarch64_dtb_scan(uint64_t dtb_pa) {
    ...
    while (off + 4U <= struct_size) {
        ...
        if (token == AARCH64_FDT_END_NODE) {
            ...
            /* RNG200 (Pi 4 の乱数器)。/soc の下にあり、GENET と同じ形で
             * ranges を通せば物理になる (2026-09-05) */
            if (n->is_rng && !n->is_disabled && n->reg_data &&
                reg_entry_phys(nodes, depth, n->reg_data, n->reg_len, ac, sc, 0, &base, &size) &&
                base != 0 && info->rng_base == 0) {
                info->rng_base = base;
                info->rng_size = size ? size : 0x28U;
                info->flags |= AARCH64_BOOT_FLAG_RNG_FROM_DTB;
            }

            /* GENET (Pi 4 の実 NIC)。**アドレスは /scb の ranges を通す**
             * (emmc2 と同じ形。/scb は "simple-bus" で reg はそのまま
             * ranges 変換だけで物理になる)。N-9 手2 */
            if (n->is_genet && !n->is_disabled && n->reg_data &&
                reg_entry_phys(nodes, depth, n->reg_data, n->reg_len, ac, sc, 0, &base, &size) &&
                base != 0 && info->genet_base == 0) {
                info->genet_base = base;
                info->genet_size = size ? size : 0x10000U;
                info->flags |= AARCH64_BOOT_FLAG_GENET_FROM_DTB;

                if (n->intr_data && n->intr_len >= 12U &&
                    aarch64_dtb_read_be32(n->intr_data) == 0U) {
                    info->genet_intid = aarch64_dtb_read_be32(n->intr_data + 4) + 32U;
                }

                /* phy-mode の文字列を小さい整数に変換する。
                 * **順番に注意**: "rgmii-id" は "rgmii" を前方一致で含むので、
                 * 長いものから先に見る (エンジニアリングのお約束) */
                if (n->phy_mode_data && n->phy_mode_len > 0) {
                    const char* s = (const char*)n->phy_mode_data;
                    if (str_eq(s, "rgmii-id")) info->genet_phy_mode = 1;
                    else if (str_eq(s, "rgmii-rxid")) info->genet_phy_mode = 2;
                    else if (str_eq(s, "rgmii-txid")) info->genet_phy_mode = 3;
                    else info->genet_phy_mode = 0;   /* "rgmii" 含む既定 */
                }

                if (n->mac_addr_data && n->mac_addr_len >= 6U) {
                    for (int mi = 0; mi < 6; mi++) info->genet_mac[mi] = n->mac_addr_data[mi];
                    info->flags |= AARCH64_BOOT_FLAG_GENET_MAC_FROM_DTB;
                }

                /* **DMA の番地変換。** /scb の dma-ranges には複数組あり
                 * (実測: Pi 4 で 2 組)、RAM 全体を指すのは size が一番大きい
                 * 組だった (child=parent=0、16GB)。**先頭だけ読むと違う窓
                 * (56MB の狭い窓) を掴む** — emmc2 の 1 組決め打ちとはここが
                 * 違う。全部見て一番大きい size を選ぶ */
                if (parent != depth && nodes[parent].dma_ranges_data) {
                    uint32_t cac = addr_cells[parent];
                    uint32_t pac = (parent > 0) ? addr_cells[parent - 1] : addr_cells[0];
                    uint32_t csc = size_cells[parent];
                    uint32_t stride = (cac + pac + csc) * 4U;
                    if (cac <= 2 && pac <= 2 && csc <= 2 && cac != 0 && stride != 0) {
                        const uint8_t* base_q = nodes[parent].dma_ranges_data;
                        uint32_t dlen = nodes[parent].dma_ranges_len;
                        uint64_t best_span = 0, best_offset = 0;
                        uint32_t o;
                        for (o = 0; o + stride <= dlen; o += stride) {
                            const uint8_t* q = base_q + o;
                            uint64_t child, host, span;
                            child = (cac == 2) ? aarch64_dtb_read_be64(q)
                                               : aarch64_dtb_read_be32(q);
                            q += cac * 4U;
                            ...
                            if (span > best_span) {
                                best_span = span;
                                best_offset = child - host;
                            }
                        }
                        info->genet_dma_offset = best_offset;
                        info->genet_dma_limit = best_span;
                    }
                }
            }
        ...
        if (token == AARCH64_FDT_PROP) {
            ...
            if (str_eq(prop_name, "#address-cells") && len >= 4U) {
                ...
                /* Pi 4 の実 NIC。QEMU virt には名乗るノードが無いので
                 * ここが立たず、virtio-net-mmio へ自然に落ちる (N-9 手2) */
                if (compatible_has(c, len, "brcm,bcm2711-genet-v5")) n->is_genet = 1;
                /* BCM2711 の乱数器。**TLS の鍵の材料の出どころ** (2026-09-05)。
                 * 実機の Raspberry Pi OS も同じ番地を
                 * "iproc-rng200 fe104000.rng" として掴んでいる */
                if (compatible_has(c, len, "brcm,bcm2711-rng200")) n->is_rng = 1;
            ...
            } else if (str_eq(prop_name, "phy-mode")) {
                nodes[depth].phy_mode_data = data;
                nodes[depth].phy_mode_len = len;
            } else if (str_eq(prop_name, "local-mac-address")) {
                nodes[depth].mac_addr_data = data;
                nodes[depth].mac_addr_len = len;
            }
