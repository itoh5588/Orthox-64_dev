...
uint32_t aarch64_genet_intid(void);
...
static void aarch64_boot_info_dump(void) {
    ...
    /* Pi 4 の実 NIC (GENETv5、N-9 手2)。**QEMU virt には無い** ので
     * virtio-net-mmio に落ちる。emmc2 と同じ「無い/取り損ねた」を混ぜない
     * 形にした */
    aarch64_uart_puts("  genet     : ");
    if (b->genet_base == 0) {
        aarch64_uart_puts("not present on this machine\n");
    } else {
        put_hex64(b->genet_base);
        aarch64_uart_puts(" size ");
        put_hex64(b->genet_size);
        aarch64_uart_puts(" irq ");
        put_hex64(b->genet_intid);
        aarch64_uart_puts(" phy_mode ");
        put_hex64(b->genet_phy_mode);
        put_src(AARCH64_BOOT_FLAG_GENET_FROM_DTB);
        aarch64_uart_puts("  genet mac : ");
        for (int gi = 0; gi < 6; gi++) {
            put_hex64(b->genet_mac[gi]);
            if (gi != 5) aarch64_uart_puts(":");
        }
        aarch64_uart_puts((b->flags & AARCH64_BOOT_FLAG_GENET_MAC_FROM_DTB)
                          ? "  (dtb)\n" : "  (not in dtb, will use register value)\n");
        aarch64_uart_puts("  genet dma : offset ");
        put_hex64(b->genet_dma_offset);
        aarch64_uart_puts(" limit ");
        put_hex64(b->genet_dma_limit);
        aarch64_uart_puts("\n");
    }
...
void aarch64_boot_continue(void) {
    ...
    /* ---- N-9 手1: ネットワーク (QEMU virt の virtio-net-device だけ) ----
     * **x86 (kernel/init.c) と同じ位置関係。**fs の後、ユーザープロセスの
     * 前。実体は kernel/net.c (共有層) -> kernel/aarch64/virtio_net_mmio.c。
     * 実機 (Pi 4) には繋がる相手が無いので、無ければ黙って何もしない */
    {
        extern void net_init(void);
        extern int net_write_resolv_conf(void);
        net_init();
        /* **DHCP の結果を /etc/resolv.conf に落とす** (2026-09-05、TLS の手3)。
         * Linux で DHCP クライアントがやることをここでやる。これが無いと
         * musl の getaddrinfo が引く先を知らず、EL0 から名前で繋げない。
         * ネットワークが無ければ待たずに素通りする */
        if (net_write_resolv_conf()) {
            aarch64_uart_puts("net: wrote /etc/resolv.conf\r\n");
        }
        /* **壁時計を合わせる** (2026-09-05、TLS の手2)。RTC が無いので
         * SNTP で外へ聞く。出す先は /etc/ntp.conf で変えられ、空のファイルを
         * 置けば出ない。**合わなくても起動は止めない** */
        {
            /* **三項演算子で書いてはいけない。**失敗は負の値で返るので、
             * 真偽で見ると -1 まで「合った」になる */
            switch (net_sync_wallclock()) {
            case NET_CLOCK_SYNCED:
                aarch64_uart_puts("net: clock synced (SNTP)\r\n"); break;
            case NET_CLOCK_ERR_DNS:
                aarch64_uart_puts("net: clock not synced (NTP name lookup failed)\r\n"); break;
            case NET_CLOCK_ERR_NOREPLY:
                aarch64_uart_puts("net: clock not synced (no NTP reply)\r\n"); break;
            default:
                aarch64_uart_puts("net: clock not synced\r\n"); break;
            }
        }
    }

    /* ---- 乱数源 (2026-09-05、TLS の手1) --------------------------------
     * **ユーザープロセスの前に開ける。**getrandom(2) は最初のプロセスから
     * 呼ばれうる。源が無ければ 1 行出して先へ進み、getrandom は -ENOSYS を
     * 返す (**予測できる値を黙って返さない**) */
    {
        extern int aarch64_rng_init(void);
        aarch64_rng_init();
    }
...
void aarch64_irq_handler(struct aarch64_trap_frame* frame) {
    ...
    if (intid < 16U) {
        ...
        aarch64_emmc2_irq();
    } else if (intid && intid == aarch64_genet_intid()) {
        /* GENET の RX/TX 完了 (N-11)。**発生源を落として poll を回すだけ。**
         * 10ms のタイマ待ちを挟まずにリングを空けられる */
        aarch64_genet_irq();
