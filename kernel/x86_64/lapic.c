...
static uint64_t lapic_enter_kernel_address_space(void) {
    uint64_t current = arch_mmu_read_address_space();
    uint64_t kernel;
    if (!vmm_get_kernel_pml4()) {
        return current;
    }
    kernel = arch_vm_kernel_address_space();
    if (current != kernel) {
        arch_mmu_write_address_space(kernel);
    }
    return current;
}

static void lapic_leave_kernel_address_space(uint64_t previous) {
    uint64_t kernel = arch_vm_kernel_address_space();
    if (!vmm_get_kernel_pml4()) {
        return;
    }
    if (previous != kernel) {
        arch_mmu_write_address_space(previous);
    }
}

static void lapic_write(uint32_t reg, uint32_t data) {
    uint64_t previous = lapic_enter_kernel_address_space();
    lapic_base[reg >> 2] = data;
    lapic_leave_kernel_address_space(previous);
}

static uint32_t lapic_read(uint32_t reg) {
    uint32_t value;
    uint64_t previous = lapic_enter_kernel_address_space();
    value = lapic_base[reg >> 2];
    lapic_leave_kernel_address_space(previous);
    return value;
}
