...
/* **1ms 待ちの時刻源。**x86 は LAPIC のタイマ、aarch64 は generic timer から
 * 取るが、**どちらを使うかは arch_time_now_ms() が知っている**ので、ここに
 * 分岐は要らない。2026-09-07 まで x86 側だけ lapic_get_ticks_ms() を直に
 * 呼んでいた (aarch64 は kernel/aarch64/stubs.c で同名の皮を被せていた)。
 * 中身 (xHCI のリングやスロット操作) は両方で同じものが動く */
#define USB_NOW_MS() arch_time_now_ms()

/* **待ちループで CPU に譲るヒント。**x86 の pause、aarch64 の yield。
 * 命令そのものなのでアーキで分かれる */
...
static void usb_hotplug_poll_owned(void) {
    ...
    if (now >= g_usb_heartbeat_next_ms) {
        ...
        /* ---- V-1: fork にどれだけ使っているか ---------------------------
         *
         * もとは CoW を入れるかを数字で決めるための計器 (aarch64)。
         * CoW にした後 (2026-09-19) は、共有した葉の数と fork の時間を出す
         * (kernel/aarch64/vm.c)。実装が無いアーキでは弱いシンボルの空実装が
         * 使われ、0 が出る */
        {
