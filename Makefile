...
X86_BOOTSTRAP_EXIT_PROBE_ELF = out/x86-bootstrap-exit-probe.elf
...
CORE_SRCS = kernel/elf.c kernel/task.c kernel/task_exec.c kernel/task_fork.c \
	kernel/sys_mmap.c kernel/vm_cow.c \
...
RISCV64_SHARED_C_SRCS = kernel/task.c kernel/task_exec.c kernel/task_fork.c kernel/sched.c \
	kernel/linux_syscall.c kernel/sys_mmap.c kernel/vm_cow.c kernel/sys_rlimit.c kernel/sys_uname.c \
...
AARCH64_SHARED_C_SRCS = kernel/usb.c \
	...
	kernel/linux_syscall.c kernel/sys_fs.c kernel/sys_mmap.c kernel/vm_cow.c kernel/sys_rlimit.c \
...
# **段が同じページを跨ぐ ELF** を読ませる (kernel/elf.c の権限の和の検査)。
# 普通のツールチェインはここを 1 ページ空けるので、専用の台本で作る
$(RISCV64_STRADDLE_PROBE_ELF): $(BUILD_DIR)/riscv64/user/riscv64_straddle_probe.o scripts/user-riscv64-straddle.ld
	@mkdir -p $(@D)
	$(LD) -nostdlib -static -m elf64lriscv -T scripts/user-riscv64-straddle.ld $< -o $@

riscv64-straddle-probe: $(RISCV64_STRADDLE_PROBE_ELF)
...
# bootstrap task (ppid==0) の exit を実機で確かめる探針。sh.elf の位置に
# 置くのは straddle probe と同じ理由 (rootfs.img も ash も要らない)
$(BUILD_DIR)/x86/user/x86_bootstrap_exit_probe.o: user/x86_bootstrap_exit_probe.c
	@mkdir -p $(@D)
	$(CC) -target $(TARGET) -std=c11 -ffreestanding -fno-PIE -O2 -Wall -Wextra -c $< -o $@

$(X86_BOOTSTRAP_EXIT_PROBE_ELF): $(BUILD_DIR)/x86/user/x86_bootstrap_exit_probe.o
	@mkdir -p $(@D)
	$(LD) $(USER_LDFLAGS) $< -o $@

x86-bootstrap-exit-probe: $(X86_BOOTSTRAP_EXIT_PROBE_ELF)

x86-bootstrap-exit-smoke: $(KERNEL_ELF) $(X86_BOOTSTRAP_EXIT_PROBE_ELF)
	bash ./tests/x86_bootstrap_exit_smoke.sh
